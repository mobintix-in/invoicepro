...
const InvoicePDFButton = dynamic(
  () => import("@/components/InvoicePDFButton"),
  { ssr: false },
);
    ...
    getInvoice(id)
      .then(async (inv) => {
        ...
        try {
          ...
          if (profile) {
            if (!inv.template && profile.template)
              inv.template = profile.template;
            if (!inv.invoiceTheme && profile.invoiceTheme)
              inv.invoiceTheme = profile.invoiceTheme;
  ...
  async function handleTemplateChange(newTemplate: string) {
    ...
    try {
      ...
      if (typeof window !== "undefined")
        localStorage.setItem("invoice_template", newTemplate);
    } catch {}
  }
    ...
    <div className="mx-auto max-w-4xl space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-4 py-2.5 shadow-xs print:hidden">
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="text-xs font-semibold uppercase tracking-wider text-gray-400 mr-1.5">
            Template:
          </span>
          {TEMPLATES.map((tmpl) => {
